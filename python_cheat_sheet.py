...
# Add 'Application.log' to the zipfile using the defined compression type above (zipfile.ZIP_DEFLATED)
zip_file.write('Application.log', compress_type=compression_type)
...
file_details_tuple = os.path.splitext('Application.log')

file_name = file_details_tuple[0]
extension = file_details_tuple[1]

# This will print 'Application'
print file_name
...
file_name, extension = os.path.splitext('Application.log')
...
conn = MySQLdb.connect(host="myhost", port=3306, user="username", passwd="password", db="mydb")
...
dict_cursr.execute('SELECT * FROM ProductTable where productID=12506;')
...
driver      = '{SQL Server}'
server_port = 'myserver\SQM1,5432'
database    = 'mysqldb'
user_name   = 'myusername'
user_pw     = 'mypassword'

# Create connection object by using the 'connect()' function
sql_conn = pyodbc.connect(driver=driver, server=server_port, database=database, uid=user_name, pwd=user_pw)

# Create cursor object by using the 'cursor()' method on the connection object
sql_cursor = sql_conn.cursor()

# Create query
sql_query = 'SELECT name, productID FROM [mysqldb].[dbo].[ProductTable]'
...
# contain the name value and the second element of the row tuple will contain the productID value
result_set = sql_cursor.fetchall()

# Iterate over the result_set and print the name and productID for each order
for row in result_set:
    name       = row[0]
    productID      = row[1]
    print 'name:%s productID:%s' % (name, productID)
